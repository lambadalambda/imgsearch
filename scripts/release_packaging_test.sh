...
assert_contains "${package_script}" 'exec "$script_dir/imgsearch" ${ort_args[@]+"${ort_args[@]}"} -vector-backend sqlite-vector "$@"'
...
assert_contains "${package_script}" 'scripts/resolve_onnxruntime_lib.sh'
assert_contains "${package_script}" '-parakeet-onnxruntime-lib "$ort_lib"'
