...
- fix(transcribe): bound ASR audio resource usage with a 10-minute duration cap, streaming ffmpeg PCM decoding instead of full-output buffering, and deterministic over-limit errors backed by sample-count guards before feature extraction.
- security(container): default CUDA container startup to loopback bind (`127.0.0.1:8080`) and require explicit `IMGSEARCH_ADDR=0.0.0.0:8080` opt-in for remote exposure, with deployment docs now calling out API-key and reverse-proxy/TLS requirements.
- refactor(http): centralize shared `limit`/`offset`/`include_nsfw` query parsing in `internal/httputil` and reuse it across image, video, search, and live handlers while preserving existing bounds.
- refactor(tags): centralize `tags_json` decoding in `internal/tagutil` and reuse it across image/video/search APIs and worker annotation paths while preserving existing empty/malformed handling.
- security(storage): centralize item-ID parsing and stored-path cleanup helpers and harden file deletion with `dataDir` containment checks (including symlink-escape rejection).
- refactor(worker): merge single-job and per-job batch execution into a shared worker job runner so embed/annotate/transcribe paths use one implementation with consistent fail/retry/complete semantics.
- refactor(worker): extract a shared transactional helper for `index_jobs` done-state updates so image, video-annotation, and transcript completion paths all use the same `mark job done` SQL.
- refactor(search): centralize reusable NSFW SQL fragments in `internal/nsfwsql` and reuse them across image/video listing plus search/tag/tag-cloud/transcript queries while preserving grouped video filtering semantics.
- perf(embedder): wire `-llama-native-max-sequences` through to llama.cpp runtime `n_seq_max` initialization and add integration coverage that inspect metadata reports the configured sequence count.
- perf(embedder): pipeline native image preprocessing and embedding per chunk so preprocessing and GPU embedding can overlap, with cancellation-safe cleanup, serial fallback when pipeline preprocessing fails, explicit `EmbedImage` prepare/execute phase split, and focused concurrency regression tests.
