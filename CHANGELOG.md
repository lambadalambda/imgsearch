...
## Unreleased
- feat: add `EmbedImages` and worker-side grouped image embedding plumbing, plus `-llama-native-max-sequences` for Go-side batch chunking. The current native implementation still uses the stable single-image bridge internally, and the 100-image benchmark showed no throughput gain (`batch-2` ~= sequential, `batch-4` slower).
