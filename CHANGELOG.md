...
- perf(embedder): pipeline native image preprocessing and embedding per chunk so preprocessing and GPU embedding can overlap, with cancellation-safe cleanup, serial fallback when pipeline preprocessing fails, and focused concurrency regression tests.
