...
func main() {
	...
	embedderOpts := llamaCPPNativeEmbedderOptions{
		...
		AnnotationNGramSpeculation: cfg.LlamaNativeAnnotationNGramSpeculation,
	...
	runtime, err := app.NewRuntime(app.RuntimeOptions{
		...
		UploadMaxImageBytes:        int64(cfg.MaxImageUploadMB) << 20,
		UploadMaxVideoBytes:        int64(cfg.MaxVideoUploadMB) << 20,
